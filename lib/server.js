...
var Server = module.exports = function () {
    ...
    var subscriptionCounter = SubscriptionCounter(); // create a subscri.ption counter for the server that's used for assinging id's to clients.
    ...
    .on('error', function (err) {
            if (err.code == 'EADDRINUSE') {
                winston.log('warn', 'Can not listen on port %d as it\'s already in use, retrying..', context.config.stratum.port);
                setTimeout(function () {
                    server.listen(context.config.stratum.port);
                }, 5000);
            }
        });
