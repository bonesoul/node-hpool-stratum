...
var pool = exports.Pool = function (options) {
    ...
    var _this = this;
    var errorLogEmitter = function (text) { _this.emit('log', 'error'  , text); }; // error emitter to share logs with parents.
    ...
    function setupDaemon(callback) {
        if (!Array.isArray(options.daemons) || options.daemons.lenght < 1) { /* make sure we have supplied daemon configurations */
