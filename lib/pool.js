...
var pool = exports.Pool = function(config) {
    ...
    _this.poolAddress = null; // validation results for supplied pool address.
    
    // pool information & statistics.
    _this.info = {
        network: {}, // network information
        feePercent: 0, // total percent of pool fees.
    ...
    function validatePoolAddress(callback) {
        _this.daemon.cmd('validateaddress', [_this.config.wallet.address], function (response) {
            ...
            _this.poolAddress = response.result; // set the result of validateaddress(pool) 
            ...
            if (!_this.poolAddress.isvalid) { // make sure configured address is valid.
            ...
            _this.poolAddress.script = _this.config.coin.options.isProofOfStakeHybrid ? 
                utils.pubkeyToScript(_this.poolAddress.pubkey) : // pos hybrid coins need to use pubkey script within coinbase transaction.
                utils.addressToScript(_this.poolAddress.address); // pure pow coins just use the address within the coinbase transaction.
    ...
    function setupRecipients(callback) {
        ...
        for (var entry in config.rewards) {
            try {
                ...
                _this.info.recipients.push(recipient);
                _this.info.feePercent += percent;
            } catch (e) {
                emitErrorLog('Error generating transaction output script for ' + entry);
            }

            if (_this.info.feePercent === 0) {
                emitWarningLog('Your pool is configured with 0% fees!');
            }
        }
    }

    function setupJobManager(callback) {
