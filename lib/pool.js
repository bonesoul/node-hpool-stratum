...
var Pool = exports.Pool = function (config) {
    ...
    function validatePoolAddress(callback) {
        this.daemon.cmd('validateaddress', [_this.info.config.wallet.address], function (response) {
    ...
    function setupJobManager() {
        _this.jobManager = new jobManager(_this.info)
            .on('newJob', function(job) {
                emitInfoLog('New job created ' + job.id + ' for block ' + job.data.height);
            })
            .on('share', function(share) {
                
            });
    }

    function startServer(callback) {
        _this.server = new stratum.Server(_this, _this.jobManager) // create the stratum server.
            .on('client.connected', function(client) {
                client.on('submit', function (params, resultCallback) {

                    var result = _this.jobManager.processShare(client, params);
