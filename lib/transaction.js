...
exports.CreateGeneration = function (data, poolInfo) {
    ...
    var txTimestamp = poolInfo.coin.isProofOfStakeHybrid ?
        util.packUInt32LE(data.curtime) : new Buffer([]);
    
    //For coins that support/require transaction comments
    var txComment = poolInfo.config.coin.capatabilities.txMessage === true ?
    ...
    var scriptSigPart1 = Buffer.concat([
        ...
        new Buffer([poolInfo.extraNonce.placeholder.length])
    ]);
    ...
    var p1 = Buffer.concat([
        ...
        utils.varIntBuffer(scriptSigPart1.length + poolInfo.extraNonce.placeholder.length + scriptSigPart2.length),
        scriptSigPart1
    ]);
    ...
    var outputTransactions = generateOutputTransactions(data, poolInfo);    
...
var generateOutputTransactions = function (data, poolInfo) {
    ...
    for (var i = 0; i < poolInfo.fees.recipients.length; i++) {
        {
            var recipient = poolInfo.fees.recipients[i];
        ...
        txOutputBuffers.unshift(Buffer.concat([
                ...
                utils.varIntBuffer(poolInfo.wallet.central.script.length),
                poolInfo.wallet.central.script
            ]));
    }
